	...
	implements OnInit, AfterViewInit, OnChanges, OnDestroy
{
	...
	public itemList: ImageWithDataKey[] = [];
	...
	private _currentIntersecting: Set<ImageWithDataKey> = new Set();
	...
	// 	let itemList: ImageWithDataKey[];
	// 	this.marqueeItems.changes.subscribe((items: QueryList<ElementRef>) => {
	// 		itemList = this._getElementsToObserve(items);
	// 	});
	// }

	private _getElementsToObserve(items: QueryList<ElementRef>): ImageWithDataKey[] {
		console.log('Elements to observe invoked');
		const array: ImageWithDataKey[] = [];
		items.forEach(item => {
			const element = item.nativeElement as ImageWithDataKey;
			array.push(element);
		});
	...
	// private _setupIntersectionObserver(elementsToObserve: ImageWithDataKey[]) {
	...
	private _findRenderedImage(target: ImageWithDataKey): RenderedImage {
